...
def local_alignment_logprob(a, b, align_scorer=None):
    ...
    if align_scorer is None:
        align_scorer = AlignScorer()
        score = align_scorer.score_linear
    else:
        score = align_scorer.score_logprob
    ...
    for i in range(len(a)):
        ...
        for j in range(1, len(dp)):
            ...
            if ndp[j - 1][0] is not None:  # insertion
                ndp[j] = max(ndp[j], (ndp[j - 1][0] + score('ADD', b[j - 1]),
                                      ndp[j - 1][1]))

            if a[i] == b[j - 1]:  # match
                ndp[j] = max(ndp[j],
                             (score('DEL', -1) * i + score('MATCH', b[j - 1]),
                              -j))  # first match of the sequences
                if dp[j - 1][0] is not None:  # continue sequence
                    ndp[j] = max(ndp[j],
                                 (dp[j - 1][0] + score('MATCH', b[j - 1]),
                                  dp[j - 1][1]))
...
def score_idea(positions_d, pos, loffset, roffset):
    ...
    return sum(scores[:min(max(len(scores) / 5, 5), len(scores))])
...
class IdeaExtractor():
    def __init__(self, data, action_prob, N):
        self.data = data
        self.wordid_count = self.data.wordid_count
        self.action_prob = action_prob
        self.align_scorer = AlignScorer(self.action_prob, self.wordid_count)
        self.N = N
        self.clear()
...
class IdeaFinder():
    ...
    def __init__(self, data,
                 ...
                 min_idea_length=7,
                 force_longer_ideas=False,
                 max_offset_length=None):
        global DATA
        DATA = data
        self.data = data

        self.wordid_count = data.wordid_count

        if ptr_params is not None:
            self.action_prob = ptr_params.action_prob
        else:
            self.action_prob = {'MATCH': 0.8, 'ADD': 0.1, 'SUB': 0.1}
        ...
        self.force_longer_ideas = force_longer_ideas
    ...
    def clear(self):
        self.extractor = IdeaExtractor(self.data, self.action_prob, self.N)
        ...
        for ngram in ngrams:
            ...
            if len(idea) < self.min_idea_length:
                ...
                sys.stderr.write("done.\nExtracted (%d): %s\n\n" % (
                    ...
                    ' '.join(self.data.get_words_from_wordids(idea))))
                sys.stderr.flush()
...
def optimal_idea_allocation(data, ideas):
